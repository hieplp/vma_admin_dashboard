...
export default {
  ...
  methods: {
    ...
    uploadImageToFirebase(imageData, imgTypeName) {
      return new Promise((resolve) => {
        ...
        const storageRef = firebase
          .storage()
          .ref(imgTypeName + "-" + this.driver.fullName + "-" + date)
          .put(imageData);
        storageRef.on(
          `state_changed`,
          (snapshot) => {
    ...
    async initDataDocument(documentTypeId, documentRef) {
      if (this.checkDocumentSelected(documentTypeId)) {
        ...
        await this.deleteFirebaseLink(
          this.driver.userDocumentList[this.index].userDocumentImages
        );
        this.driver.userDocumentList[this.index].userDocumentImages = [];
        ...
        if (oldIndex >= 0) {
          this.$delete(this.driver.userDocumentList, oldIndex);
        }
      }
    },
    ...
    getIndexOfDocument(userDocumentType) {
      for (
        let index = 0;
        index < this.driver.userDocumentList.length;
        index++
      ) {
        let document = this.driver.userDocumentList[index].userDocumentType;
